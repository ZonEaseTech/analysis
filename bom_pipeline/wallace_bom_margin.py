...
# 规则(何伟涛 2026-06): 外卖无大包酱料 / 堂食无小包酱料。按渠道丢错包酱料行(不替换)。
def drop_wrong_sauce(items, cb):
    if not items:
        return items
    bad = "大包" if cb == "外卖" else "小包"
    return [it for it in items if bad not in str(it[0])]


DINEIN_BAGS = {"打包袋 (大号)", "打包袋 (小号)", "打包袋(小号)", "大号打包袋",
               "外卖筒形薯盒", "酱料盒（外卖）", "米饭打包碗（圆形）",
               "打包袋(大号)"}

DINEIN_REMOVE_MATS = DINEIN_BAGS | {
    "单杯袋", "哑光塑料封口膜", "手套", "一次性手套", "托盘纸", "一次性餐包",
}

TAKEOUT_SINGLE_REMOVE = {"托盘纸", "酱料盒（堂食）"}

PRODUCT_SPECIAL = {
    ("堂食", "照烧脆皮鸡"): {"玉米粒", "生菜", "Teriyaki Sauce"},
    ("外卖", "鸡米花饭"): {"黄瓜片", "中薯盒", "16A 冷杯", "百事可乐原味", "PLA尖头吸管"},
}

def drop_dinein_packaging(items, cb):
    """堂食不需要外卖包材"""
    if not items or cb != "堂食":
        return items
    return [it for it in items if str(it[0]) not in DINEIN_REMOVE_MATS]

def drop_takeout_single(items, kind):
    """外卖单品去托盘纸/酱料盒(堂食)"""
    if not items or kind != "single":
        return items
    return [it for it in items if str(it[0]) not in TAKEOUT_SINGLE_REMOVE]

def drop_product_bom(items, nmd, cb):
    """商品专属BOM删除"""
    if not items:
        return items
    for (ch, keyword), mats in PRODUCT_SPECIAL.items():
        if cb == ch and keyword in nmd:
            items = [it for it in items if str(it[0]) not in mats]
    return items
...
def resolve_recon_path(sales_path, recon_path=None):
    ...
    if not month:
        return None
    candidate = os.path.join(BASE, f"Wallace门店实收明细_{month}.json")
